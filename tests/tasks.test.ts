...
describe("Tasks API", () => {
  ...
  it("GET /tasks - should return all tasks", async () => {
    const response = await request(serverApp).get("/tasks").expect(200);
  ...
  it("GET /tasks/:id - should return task by ID", async () => {
    const response = await request(serverApp).get(`/tasks/${taskId}`).expect(200);
  ...
  it("GET /tasks/:id - should return 404 if task not found", async () => {
    const response = await request(serverApp).get(`/tasks/9999`).expect(404);
  ...
  it("DELETE /tasks/:id - should delete task by ID", async () => {
    const response = await request(serverApp).delete(`/tasks/${taskId}`).expect(200);

    expect(response.body).toHaveProperty("message", "Task successfully deleted");
    expect(response.body.result).toBe(true);

    await request(serverApp).get(`/tasks/${taskId}`).expect(404);
  });

  it("DELETE /tasks/:id - should return 404 if task not found", async () => {
    const response = await request(serverApp).delete(`/tasks/9999`).expect(404);
    expect(response.body).toHaveProperty("message", "Task not found");
  });
});