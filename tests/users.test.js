...
describe("Users API", () => {
    ...
    it("GET /users/:id - should return user by ID", async () => {
        const response = await request(app).get(`/users/${romanId}`).expect(200);
    ...
    it("GET /users/:id - should return 404 if user not found", async () => {
        const response = await request(app).get(`/users/9999`).expect(404);
    ...
    it("DELETE /users/:id - should delete user by ID", async () => {
        const response = await request(app).delete(`/users/${romanId}`).expect(200);
        ...
        await request(app).get(`/users/${romanId}`).expect(404);
    });
    it("DELETE /users/:id - should return 404 if user not found", async () => {
        const response = await request(app).delete(`/users/9999`).expect(404);
        expect(response.body).toHaveProperty("message", "User not found");
    });
});
