...
class UsersApi {
  ...
  constructor(apiUrl: string = "http://localhost:3000/users") {
    this.apiUrl = apiUrl;
  }

  findUserById = async (id: string): Promise<UserCreate | null> => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `User with id ${id} not found (status: ${response.status})`,
        );
      }
  ...
  fetchAllUsers = async (): Promise<UserCreate[]> => {
    try {
  ...
  updateUserInfo = async (
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to update user with id ${id} (status: ${response.status})`,
        );
      }

      const updatedUser: UserCreate = await response.json();
      return updatedUser;
    } catch (error) {
  ...
  deleUserById = async (id: string): Promise<boolean> => {
    try {
      const user = await this.findUserById(id);
